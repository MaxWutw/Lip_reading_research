...
def train_net(args):
    ...
    logger = get_logger()
    model = torch.load('grid_transformer.pkl')
...
#                torch.save(model, 'grid_transformer.pkl')
                epochs_since_improvement = 0
...
def train(train_loader, model, optimizer, epoch, logger, k):
    ...
    for i, (data) in enumerate(train_loader):
        ...
        pred, gold, attn_score = model(padded_input, padded_target)

#        print(attn_score)
#        print('attention shape', attn_score[0].shape)
#        input('waiting......')

        cols = 2
        rows = 4
        attentions_pos = attn_score[0].detach().cpu().numpy()
        ...
        plt.savefig('attention_heatmap_trained.png')
        input('waiting.......')
